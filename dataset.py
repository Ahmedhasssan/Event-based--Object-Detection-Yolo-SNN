...
class NCaltech101_ObjectDetection(NCaltech101):
    ...
    def createDataset(self):
        ...
        for i_class, object_class in enumerate(self.object_classes):
            ...
            #dir_path = os.path.join(self.root, 'Caltech101')
...
class NCars(NCaltech101):
    ...
    def __getitem__(self, idx):
        ...
        #with open(os.path.join(folder, filename), "rb") as fp:
          time, coords, polarity = dataloader.read_events.load_atis_events(fp)
          x,y=coords.T
          
        events= np.stack((x, y, time, polarity), axis=-1).astype(np.int64)
        #import pdb;pdb.set_trace()
        #events = np.load(filename).astype(np.float32)
        nr_events = events.shape[0]
        ...
        #for a in self.object_classes:
        #  folder=os.path.join(self.root, a)
        ...
        #      time, coords, polarity = dataloader.read_events.load_atis_events(fp)
        #      x,y=coords.T
        #      events= np.stack((x, y, time, polarity), axis=-1)
        #      histogram = self.generate_input_representation(events, (self.height, self.width))
        #      
        #      return events, label, histogram
    ...
    #    #label = np.loadtxt(os.path.join(self.root, self.files[idx])).astype(np.int64)
    #    events = np.loadtxt(os.path.join(self.root, self.files[idx], 'events.txt'), dtype=np.float32)
    #    #events = np.loadtxt(os.path.join(self.root, self.files[idx]), dtype=np.float32)
