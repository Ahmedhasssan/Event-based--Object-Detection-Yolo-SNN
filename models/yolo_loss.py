...
def TET_loss(outputs, labels, means=1, lamb=0.75):
    #T = outputs.size(1)
    #Loss_es = 0
    criterion = torch.nn.CrossEntropyLoss()
    #for t in range(T):
    Loss_es = criterion(outputs, labels)
...
def yoloLoss(model_output, bounding_box, input_shape):
    ...
    #batch_s, gt_nr_bbox, _ = np.shape(bounding_box)
    ...
    #cell_map_shape = torch.tensor(model_output.shape[1:4], device=model_output.device)
    ...
    class_loss=TET_loss(pred_class_bbox, class_label)
    #class_loss = loss_function(pred_class_bbox, target=class_label)
    #print(len(pred_class_bbox))
