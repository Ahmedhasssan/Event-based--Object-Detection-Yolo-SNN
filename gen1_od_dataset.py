...
class GEN1DetectionDataset(Dataset):
    ...
    def create_targets(self, boxes):
        ...
        bboxes = structured_to_unstructured(boxes)[:, [1, 2, 3, 4, 5]]
        np_bbox = self.cropToFrame(bboxes)
        ...
        const_size_bbox = np.zeros([45, 5])
        const_size_bbox[:np_bbox.shape[0], :] = np_bbox
        
        return const_size_bbox.astype(np.int64)
    ...
    #     for i,a in enumerate(torch_labels):
    #         labels = torch.cat((torch_boxes[i], torch.tensor([a])),0)
    #         ground_truths = torch.vstack((ground_truths,labels))
